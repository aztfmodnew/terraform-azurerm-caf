...
module "app_service_environments_v3" {
  ...
  global_settings     = local.global_settings
  settings            = each.value
  subnet_id           = can(each.value.subnet_id) ? each.value.subnet_id : coalesce(try(local.combined_objects_virtual_subnets[try(each.value.vnet.lz_key, local.client_config.landingzone_key)][each.value.vnet.subnet_key].id, null), try(local.combined_objects_networking[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.vnet_key].subnets[each.value.subnet_key].id, null))
