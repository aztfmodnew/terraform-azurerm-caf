...
resource "azurecaf_name" "managed_redis" {
  name          = try(var.settings.name_override, null) != null ? var.settings.name_override : var.settings.name
  resource_type = "azurerm_redis_cache"
  prefixes      = var.global_settings.prefixes
  random_length = var.global_settings.random_length
  clean_input   = true
  passthrough   = try(var.settings.name_override, null) != null ? true : var.global_settings.passthrough
