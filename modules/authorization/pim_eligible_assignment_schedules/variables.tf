variable "settings" {
  ...
  Settings object for the PIM eligible assignment schedule. Configuration attributes:
    ...
    - role_definition_name  - (Required) Display name of the role definition to assign (e.g., "Owner", "Contributor").
  ...
  validation {
    ...
    error_message = "Unsupported attributes in settings. Allowed attributes are: name, scope, role_definition_name, principal_id, justification, start_date, end_date, ticket_number, ticket_system, permanent, tags."
  }
}
