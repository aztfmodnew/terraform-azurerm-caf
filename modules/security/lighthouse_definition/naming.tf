# Hybrid naming system for azurerm_lighthouse_assignment
...
locals {
  ...
  resource_type = "azurerm_role_definition" # Use closest supported type instead of general
...
resource "azurecaf_name" "main_resource" {
  ...
  name          = local.base_name
  resource_type = "azurerm_resource_group" # Using temporary workaround as azurerm_lighthouse_assignment is not supported by azurecaf
