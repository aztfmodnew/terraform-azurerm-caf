...
locals {
  ...
  billing_scope_by_key  = try(var.billing_scope_id, null)
  billing_scope_id      = coalesce(local.billing_scope_by_key, local.billing_scope_by_name)
}
...
resource "azurerm_subscription" "sub" {
  ...
  billing_scope_id  = can(local.billing_scope_id) ? local.billing_scope_id : try(var.settings.billing_scope_id, null)
