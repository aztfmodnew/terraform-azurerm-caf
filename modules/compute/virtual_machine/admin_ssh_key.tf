...
locals {
  ...
  create_sshkeys = ((local.os_type == "linux" || local.os_type == "legacy") && can(var.settings.public_key_pem_file) == false &&
    ...
    try(var.settings.virtual_machine_settings["linux"].disable_sshkey_generation, false) == false &&
    try(var.settings.virtual_machine_settings["legacy"].os_profile_linux_config.disable_sshkey_generation, false) == false
  )
}
