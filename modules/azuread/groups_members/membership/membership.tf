resource "azuread_group_member" "group_ids" {
  ...
  for_each = length(try(var.azuread_groups,[])) > 0 && length(try(var.members.keys, [])) > 0 ? toset(try(var.members.keys, [])) : []
...
resource "azuread_group_member" "ids" {
  ...
  for_each = length(try(var.azuread_service_principals,[])) > 0 && length(try(var.members.keys, [])) > 0 ? toset(try(var.members.keys, [])) : []
...
resource "azuread_group_member" "msi_ids" {
  ...
  for_each = length(try(var.managed_identities,[])) > 0 && length(try(var.members.keys, [])) > 0 ? toset(try(var.members.keys, [])) : []
...
# for_each = var.mssql_servers != {} ? toset(try(var.members.keys, [])) : []
  for_each = length(try(var.mssql_servers,[])) > 0 && length(try(var.members.keys, [])) > 0 ? toset(try(var.members.keys, [])) : []
