# Hybrid naming system for azurerm_service_plan
...
resource "azurecaf_name" "main_resource" {
  ...
  name          = local.base_name
  resource_type = "azurerm_resource_group" # Using temporary workaround as azurerm_service_plan is not supported by azurecaf
