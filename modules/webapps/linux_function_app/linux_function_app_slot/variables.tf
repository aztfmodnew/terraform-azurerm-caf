...
- backup { name, schedule { frequency_interval, frequency_unit, keep_at_least_one_backup, retention_period_days, start_time }, storage_account_url, storage_account { key, lz_key }, enabled }
...
- identity { type, identity_ids, managed_identity_keys, remote { managed_identity_keys }, lz_key }
...
Compatibility notes:
- Module currently reads compatibility paths under auth_settings.microsoft_v2 and auth_settings.twitter_v2 for some v2 fields.
- Module currently expects application_insigths_key (spelling preserved for backward compatibility).
