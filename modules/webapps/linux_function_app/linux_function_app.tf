...
#(Optional) Zero or more custom_oidc_v2 blocks as defined below.
  ...
  virtual_network_subnet_id = coalesce(
    ...
    try(var.remote_objects.virtual_network_subnets[try(var.settings.virtual_network_subnet.lz_key, var.client_config.landingzone_key)][var.settings.virtual_network_subnet.key].id,null),
    try(var.remote_objects.virtual_networks[try(var.settings.virtual_network_subnet.lz_key, var.client_config.landingzone_key)][var.settings.virtual_network_subnet.vnet_key].subnets[var.settings.virtual_network_subnet.subnet_key].id, null)
    )
