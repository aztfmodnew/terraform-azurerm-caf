# Hybrid naming system for azurerm_static_web_app_custom_domain
...
resource "azurecaf_name" "main_resource" {
  ...
  name          = local.base_name
  resource_type = "azurerm_resource_group" # Using temporary workaround as azurerm_static_web_app_custom_domain is not supported by azurecaf
