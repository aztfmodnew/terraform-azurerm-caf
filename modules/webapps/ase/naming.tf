# Hybrid naming system for azurerm_resource_group_template_deployment
...
resource "azurecaf_name" "main_resource" {
  ...
  name          = local.base_name
  resource_type = "azurerm_app_service_environment" # Use supported ASE type
...
# ASE-specific naming resource for backward compatibility
