# Hybrid naming system for azurerm_vpn_gateway_nat_rule
...
resource "azurecaf_name" "main_resource" {
  ...
  name          = local.base_name
  resource_type = "azurerm_virtual_network_gateway" # Use closest supported type instead of general 
  # "azurerm_vpn_gateway_nat_rule" is not a valid resource type in AzureCAF
