...
module "special_subnets" {
  ...
  resource_group_name                           = local.resource_group_name
  private_endpoint_network_policies             = try(each.value.private_endpoint_network_policies, each.value.enforce_private_link_endpoint_network_policies == true ? "Disabled" : each.value.enforce_private_link_endpoint_network_policies == false ? "Enabled" : null, null)
